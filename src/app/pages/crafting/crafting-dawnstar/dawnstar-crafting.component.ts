...
@Component({
  ...
  templateUrl: './dawnstar-crafting.component.html',
  styleUrls: ['./dawnstar-crafting.component.css'],
  animations: [
...
export class DawnstarCraftingComponent implements OnInit {
  recipes: Recipe[] = [];
  items: Item[] = [];
  ...
  ngOnInit() {
    forkJoin({
      ...
      items: this.inventoryService.getAllItems().pipe(
        map(items => items.filter(item => item.location === 'Dawnstar'))
      ),
      potions: this.potionService.getAllPotions().pipe(
        map(potions => potions.filter(potion => potion.location === 'Dawnstar'))
      )
      ...
      this.recipes.forEach(recipe => {
        ...
        recipe.amountInDawnstar = potions
